...
# FIX 7: Network Timeout added to prevent system hangs
...
FIX 3: Bayesian Smoothing Probability Engine
...
if len(closed_trades) < 10: return None # Need minimal global baseline  
...
C = 30 # Confidence parameter (Requires ~30 trades to overpower the global average)  
  
# Bayesian Formula  
bayesian_prob = (W + C * global_mu) / (N + C)  
...
st.set_page_config(page_title="Maddy AI Pro V15", layout="wide")
st.title("🦅 Maddy AI: V15.0 (The Apex Architecture)")
st.markdown("🔥 Bayesian Prob | Compounding Equity | DB Indexes | Full Traceback Logs")
...
st.sidebar.info("Next Phase: Migrate YFinance to Kite API for High-Frequency Scaling.")
...
# FIX 6: Real Compounding Equity Calculation  
total_realized_pnl = master_db['Net_PnL'].sum() if not master_db.empty else 0  
        ...
        if row['Status'] == 'Open':  
            ...
            for date, candle in future_data.iterrows():  
                ...
                if status:  
                    ...
                    charges = turnover * 0.001 # STT/Brokerage proxy  
...
updated_net_pnl = closed_trades['Net_PnL'].sum() if not closed_trades.empty else 0  
...
# FIX 2: Penalty for bad regimes instead of hard skip  
...
for idx, stock in enumerate(watchlist):  
    try:  
        ...
        if is_bull_market and (stock_return / nifty_return) < 1.2: continue   
        elif not is_bull_market and stock_return <= 0: continue   
        ...
        if score >= 60:  
            ...
            sl_multiplier = 1.5 if (stock_return > nifty_return * 2) else 2.5  
    ...
    # FIX 8: Full Traceback Logging for Production Debugging  
    except Exception:  
